...
export default function MyTrips() {
  ...
  const handleDelete = async (id) => {
    const confirmDelete = window.confirm("Confirmer la suppression de ce voyage ?");
  ...
  return (
    <div className="px-4 py-10 max-w-5xl mx-auto space-y-10 text-white relative">
      ...
      {trips.length === 0 ? (
        ...
        trips.map((trip) => (
          <div key={trip.id} className="bg-card rounded-2xl p-6 md:p-10 shadow-lg space-y-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div className="space-y-3">
                <h2 className="text-2xl font-bold text-secondary">
                  Voyage créé le {new Date(trip.created_at).toLocaleDateString("fr-FR")}
                </h2>
                ...
                <p className="text-gray-300">
                  ...
                  <span className="block">
                    <strong>Départ</strong> : {trip.trip_data?.startDate || "N/A"}
                  </span>
                  <span className="block">
                    <strong>Étapes</strong> : {trip.trip_data?.steps?.length || 0}
                  </span>
                </p>
